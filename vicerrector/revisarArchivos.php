...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id_archivo'])) {
    ...
    $usuario_aprueba = $_POST['usuario_aprueba'];
    $estado = $_POST['estado'];

    // Actualizar el archivo en la base de datos
    $sql = "UPDATE archivo SET observacion2 = ?, aprobado = ?, usuario_aprueba = ?, estado = ? WHERE id_archivo = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssi", $observacion2, $aprobado, $usuario_aprueba, $estado, $id_archivo);
...
// Obtener todos los archivos subidos por todos los usuarios, excluyendo los que tienen "corregir" o "revisado" en revisado y "si" o "no" en autorizado
$sql = "SELECT u.nombre_usuario, u.cargo, a.* 
        FROM archivo a 
        JOIN usuario u ON a.id_usuario = u.id_usuario 
        WHERE a.revisado NOT IN ('corregir','en espera','corregido') and a.aprobado NOT IN ('si','no') 
...
<body>
                        ...
                        <tr>
                            ...
                            <td class="btn-actions">
                                ...
                                <button class="btn btn-sm btn-warning" onclick="modificarArchivo(
                                    ...
                                    '<?php echo htmlspecialchars($fila['observacion2'] ?? ''); ?>',
                                    ...
                                    '<?php echo htmlspecialchars($fila['revisado'] ?? ''); ?>',
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="modificarArchivoForm">
                            <div class="mb-3">
                                <label for="observacion2" class="form-label">Observación 2</label>
                                <textarea class="form-control" id="observacion2" name="observacion2" rows="3" required></textarea>
                            </div>
                            <div class="mb-3">
                                <label for="aprobado" class="form-label">Aprobado</label>
                                <select class="form-select" id="aprobado" name="aprobado" required onchange="toggleUsuarioAprueba()">
                                    <option value="si">Sí</option>
                                    <option value="no">No</option>
                                </select>
                            </div>
                            <div class="mb-3" id="usuarioApruebaContainer" style="display: none;">
                                <label for="usuario_aprueba" class="form-label">Usuario Aprueba</label>
                                <input type="text" class="form-control" id="usuario_aprueba" name="usuario_aprueba" readonly>
                            </div>
                            <div class="mb-3">
                                <label for="estado" class="form-label">Estado</label>
                                <select class="form-select" id="estado" name="estado" required>
                                    <option value="revision rector">Revisión Rector</option>
                                    <option value="ultima correccion">Última Corrección</option>
                                </select>
                            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            window.modificarArchivo = function(idArchivo, observacion2, aprobado, usuario_aprueba, estado) {
                document.getElementById('id_archivo').value = idArchivo;
                document.getElementById('observacion2').value = observacion2;
                document.getElementById('aprobado').value = aprobado;
                document.getElementById('usuario_aprueba').value = usuario_aprueba;
                document.getElementById('estado').value = estado;
                toggleUsuarioAprueba(); // Llamar a la función para mostrar/ocultar el campo "Usuario Aprueba"
            ...
            // Función para mostrar/ocultar el campo "Usuario Aprueba" basado en la selección del campo "Aprobado"
            window.toggleUsuarioAprueba = function() {
                var aprobado = document.getElementById('aprobado').value;
                var usuarioApruebaContainer = document.getElementById('usuarioApruebaContainer');

                if (aprobado === 'si') {
                    usuarioApruebaContainer.style.display = 'block';
                    document.getElementById('usuario_aprueba').value = '<?php echo htmlspecialchars($nombre_usuario); ?>';
                } else {
                    usuarioApruebaContainer.style.display = 'none';
                    document.getElementById('usuario_aprueba').value = '';
                }
            }
        });
    </script>
