...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id_archivo'])) {
    ...
    $sql = "UPDATE archivo SET observacion3 = ?, estado = ?, observaciones = ?, fecha_entrega = ?, usuario_autoriza = ?, autorizado = ? WHERE id_archivo = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssi", $observacion3, $estado, $observaciones, $fecha_entrega, $usuario_autoriza, $autorizado, $id_archivo);
...
$sql = "SELECT u.nombre_usuario, u.cargo, a.* 
        ...
        WHERE a.revisado NOT IN ('corregir','corregido') and a.aprobado NOT IN ('no','en espera') and a.autorizado NOT IN ('no')
...
<body>
                    ...
                    <?php while ($fila = $resultado->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td class="btn-actions">
                                ...
                                <button class="btn btn-sm btn-warning" onclick="modificarArchivo(
                                    '<?php echo htmlspecialchars($fila['id_archivo'] ?? ''); ?>',
                                    '<?php echo htmlspecialchars($fila['observacion3'] ?? ''); ?>',
                                    '<?php echo htmlspecialchars($fila['estado'] ?? ''); ?>',
                                    '<?php echo htmlspecialchars($fila['observaciones'] ?? ''); ?>',
                                    '<?php echo htmlspecialchars($fila['fecha_entrega'] ?? ''); ?>',
                                    '<?php echo htmlspecialchars($fila['aprobado'] ?? ''); ?>',
                                    '<?php echo htmlspecialchars($fila['usuario_autoriza'] ?? ''); ?>',
                                    '<?php echo htmlspecialchars($fila['autorizado'] ?? ''); ?>'
                                )">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="modificarArchivoForm">
                            <div class="mb-3">
                                <label for="observacion3" class="form-label">Observación 3</label>
                                <textarea class="form-control" id="observacion3" name="observacion3" rows="3" required></textarea>
                            </div>
                            <div class="mb-3">
                                <label for="autorizado" class="form-label">Autorizado</label>
                                <select class="form-select" id="autorizado" name="autorizado" required onchange="toggleAutorizado()">
                                    <option value="si">Sí</option>
                                    <option value="no">No</option>
                                </select>
                            </div>
                            <div class="mb-3" id="usuarioAutorizaContainer" style="display: none;">
                                <label for="usuario_autoriza" class="form-label">Usuario Autoriza</label>
                                <input type="text" class="form-control" id="usuario_autoriza" name="usuario_autoriza" readonly>
                            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            window.modificarArchivo = function(idArchivo, observacion3, estado, observaciones, fecha_entrega, usuario_autoriza, autorizado) {
                document.getElementById('id_archivo').value = idArchivo;
                document.getElementById('observacion3').value = observacion3;
                document.getElementById('estado').value = estado;
                document.getElementById('observaciones').value = observaciones;
                document.getElementById('fecha_entrega').value = fecha_entrega;
                document.getElementById('usuario_autoriza').value = usuario_autoriza;
                document.getElementById('autorizado').value = autorizado;
                toggleAutorizado(); // Llamar a la función para mostrar/ocultar los campos basados en "Autorizado"
            ...
            // Función para mostrar/ocultar los campos basados en la selección del campo "Autorizado"
            window.toggleAutorizado = function() {
                var autorizado = document.getElementById('autorizado').value;
                var usuarioAutorizaContainer = document.getElementById('usuarioAutorizaContainer');
                var estadoContainer = document.getElementById('estadoContainer');

                if (autorizado === 'si') {
                    usuarioAutorizaContainer.style.display = 'block';
                    estadoContainer.style.display = 'block';
                    document.getElementById('usuario_autoriza').value = '<?php echo htmlspecialchars($nombre_usuario); ?>';
                } else {
                    usuarioAutorizaContainer.style.display = 'none';
                    estadoContainer.style.display = 'block';
                    document.getElementById('usuario_autoriza').value = '';
                }
            }

            // Función para establecer la fecha y hora actual de Quito, Ecuador
            window.setFechaEntrega = function() {
                var fechaEntrega = new Date().toLocaleString("en-US", {
                ...
                document.getElementById('fecha_entrega').value = `${anio}-${mes}-${dia}T${hora}:${minutos}`;
            }

            // Establecer la fecha y hora actual al cargar el modal
            $('#modificarArchivoModal').on('show.bs.modal', function() {
                setFechaEntrega();
            });
        });
    </script>
